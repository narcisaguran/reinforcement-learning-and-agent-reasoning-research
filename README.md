...
| **SPLADE:** Sparse Lexical and Expansion Model for First Stage Ranking | [Read Paper](https://arxiv.org/pdf/2107.05720)| Introduces the idea of term expansion. Trade-off between the vector sparsity of space methods like BM25 and dense methods. It uses a pre-trained BERT model and a Masked Language Modeling head. It is necessary to retrain the SPLADE model to bridge domain gaps. Not suitable for cross-lingual applications, where for SPLADE it is necessary to identify a lexical overlap between the query and documents in the database. | Jul 2021 SIGIR |
...
| LEXam: Benchmarking Legal Reasoning on 340 Law Exams | [Read Paper](https://arxiv.org/abs/2505.12864)| SOTA models struggle with multi-step reasoning and applying legal rules and principles to novel scenarios. Legal resoning often involves low lexical similarity. Uses lighteval framework to standardize the evaluation of conventional LLMs with different endpoints. Uses Alternative Annotator Test (LLM-as-a-Judge) to judge the correctness of the responses to the queries. | ICLR 2026 |
...
| The Alternative Annotator Test for LLM-as-a-Judge: How to Statistically Justify Replacing Human Annotators with LLMs | [Read Paper](https://arxiv.org/abs/2501.10970)| Many studies employing LLM annotations do not explicitly measure the alignment between LLMs and humans. Those that do use measures like accuracy, F1 score, Inter-Annotator-Agreement (IAA) kappas, and correlation. This work proposes a statistical procedure, the alt-test. Compares the LLM response against human annotators, by excluding one human annotator at a time, and checking whether the LLM probability exceeds the human probability, by calculating a winning rate. If the winning rate is greater than 0.5, they conclude that the LLM is more likely to hold an advantage over human annotators. | Aug 2025 |
| Unsupervised Dense Information Retrieval with Contrastive Learning | [Read Paper](https://arxiv.org/pdf/2112.09118)| Large annotated datasets are generally not available in languages other than English. The alternative of transfer learning is unsupervised learning; train dense retrievers without supervision and match the performance of BM25. The contrastive learning brings similar embeddings closer together in the representation space, while dissimilar ones are brought further apart. It leverages the MoCo technique to generate training pairs through cropping independent samples of text from the same document.| Aug 2022 |
| **M3-Embedding**: Multi-Linguality, Multi-Functionality, Multi-Granularity Text Embeddings Through Self-Knowledge Distillation | [Read Paper](https://arxiv.org/pdf/2402.03216)| BGE-M3, Versatile model in terms of working languages, retrieval functionalities, and input granularity. Generates versatile embeddings: dense retrieval, sparse retrieval, and multi-vector retrieval; can process different input granularities, from short inputs like sentences and passages to long documents of up to 8192 input tokens. Dataset: MIRACL, MKQA. Technical contribution: self-knowledge distillation, efficient batching, and high-quality curation of data. The model leads to superior performances on multi-lingual retrieval, cross-lingual retrieval, and multi-lingual long-document retrieval tasks. | Dec 2025 |
| Text Embeddings by Weakly-Supervised Contrastive Pre-training | [Read Paper](https://arxiv.org/abs/2212.03533)| E5 is the first model that outperforms the strong BM25 baseline on the BEIR retrieval benchmark without using any labeled data. When fine-tuned, E5 obtains the best results on the **MTEB** benchmark, beating existing embedded models with 40x more parameters. Uses partial contrastive learning where the passages from other pairs in a batch serve as negative samples. It outperforms MoCo techniques when the batch size is large enough. Further training on labeled data can inject human knowledge into the model to boost the performance. | Feb 2024 |
